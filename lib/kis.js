(function() {
  ...
  argv = opti.usage('Examples: \n  kis new\n  kis build\n  kis build -d path/to/project\n  kis serve -p 8080\n\nCommands:\n  new     Create new base project\n  build   Build your project\n  watch   Wacth and rebuild your project\n  serve   Start a simple HTTP server on port 3000, watch and build your project')["default"]({
    p: 3000,
    d: '.'
  }).describe('p', 'Port to start simple webserver on').describe('d', 'Directory to run commands against').describe('force', 'Force an action, regardless of warnings').argv;
  ...
  Kis = (function() {
    ...
    Kis.prototype.readFileSync = function(file) {
      if (fs.fileExistsSync(file)) return fs.readFileSync(file);
      return false;
    };
    ...
    Kis.prototype.stdLog = function(err, stdout, stderr) {
      ...
      if (err !== null) return console.log('exec error: ' + err);
    };

...
(function(exports) {
  ...
  Kis._readConfig = function() {
    config = findConfig('.', 'kis-config.js');
    console.log(config)
  }

  // Reverse walk the directory tree to locate the config file.
  function findConfig(path, configFile) {
    var resolvedPath = fpath.resolve(fpath.normalize(path + '/' + configFile));

    if(fs.existsSync(resolvedPath)) {
      return require(resolvedPath)
    }

    if(fpath.resolve(path) !== '/')
      return findConfig(fpath.resolve(fpath.normalize(path + '/../')), configFile)
